...
const List = ({ user, setUser, setFavItemCount, favItemCount }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(
          "https://castemerenfts-backend.onrender.com/products"
        );
  ...
  const favoritesLogued = allProducts?.filter((product) =>
  ...
  const deleteFavorite = async (ids) => {
    ...
    try {
      await axios.delete(
        `https://castemerenfts-backend.onrender.com/users/deleteFavorite/${userId}/${productId}`
      );
      // Filtrar los favoritos del usuario para eliminar el producto con el ID proporcionado
      const updatedFavorites = user?.loguedUser.userFounded.favorite.filter(
        (id) => id.toString() !== ids
      );
